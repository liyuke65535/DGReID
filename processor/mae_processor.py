    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, informations in enumerate(train_loader):
            ...
            with amp.autocast(enabled=True):
                ...
                inpaint_loss = nn.MSELoss()(x_, labels)
                # inpaint_loss = 0.0
                # loss_reid = 0.0
                loss = inpaint_loss + loss_reid
            ...
            if (n_iter + 1) % log_period == 0:
                logger.info("Epoch[{}] Iteration[{}/{}] Loss: {:.3f}, inpaint_loss: {:.3f}, Acc: {:.3f}, Base Lr: {:.2e}"
                .format(epoch, n_iter+1, len(train_loader),
                loss_meter.avg, inpaint_loss_meter.avg, acc_meter.avg, scheduler._get_lr(epoch)[0]))
        ...
        if epoch % cfg.SOLVER.VIS_PERIOD == 0:
            inpaint_vis(cfg, model, train_dir, os.path.join(log_path, "vis"), epoch)
        torch.cuda.empty_cache()
