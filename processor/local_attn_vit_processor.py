...
def local_attention_vit_do_train_with_amp(cfg,
    ...
    logger.info("saving tblog to {}".format(tb_path))
    ...
    if cfg.MODEL.PC_LOSS:
        ...
        for i, informations in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                # compute output
                _, _, feat_list = model(input)
                patch_centers.get_soft_label(path, feat_list, vid=vid, camid=camid)
        t_init = time.time() - t_init
        logger.info('initialization time: {:.2f}s'.format(t_init))
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, informations in enumerate(train_loader):
            ...
            with amp.autocast(enabled=True):
                score, global_feat, feat_list = model(img)
                
                ############## patch learning ######################
                patch_agent, position = patch_centers.get_soft_label(img_path, feat_list, vid=vid, camid=camid)
                l_ploss = cfg.MODEL.PC_LR
                if cfg.MODEL.PC_LOSS:
                    if isinstance(feat_list, list):
                        feat = torch.stack(feat_list, dim=0)
                    else:
                        feat = feat_list.transpose(0,1)
                    feat = feat[:,::1,:]
                    '''
                    ...
                    (ID + Triplet loss)
                    '''
                    reid_loss = loss_fn(score, global_feat, target, all_posvid=all_posvid, soft_label=cfg.MODEL.SOFT_LABEL, soft_weight=cfg.MODEL.SOFT_WEIGHT, soft_lambda=cfg.MODEL.SOFT_LAMBDA)
                else:
                    ploss = torch.tensor([0.]).cuda()
                    reid_loss = loss_fn(score, global_feat, target, soft_label=cfg.MODEL.SOFT_LABEL)
