...
def vit_only_cls_do_train_with_amp(cfg,
             model,
             center_criterion,
             train_loader,
             val_loader,
    ...
    for epoch in range(1, epochs + 1):
        ...
        # only cls for finetuning
        if cfg.MODEL.ONLY_CLS:
            for name, mod in model.base.named_parameters():
                if any(s in name for s in ["norm"]):
                    # print(name)
                    # continue
                    mod.requires_grad = False
                # print(name)
            print("========= finetune cls token only =========")
        model.train()
