...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        # self.norm = nn.BatchNorm1d(hidden_features,affine=True)
        # self.norm = nn.LayerNorm(hidden_features, eps=1e-6) # bad

    def forward(self, x):
        x = self.fc1(x)
        x = self.act(x)
        # x = self.norm(x) ### for test
        x = self.drop(x)
        # x = self.norm(x.transpose(1,2)).transpose(1,2) ##### for test
        x = self.fc2(x)
        x = self.drop(x)
        return x
...
class SelfEnsembleViT(nn.Module):
    """ Transformer-based Object Re-Identification
    """
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, **kwargs):
    ...
    def forward_features(self, x):
        ...
        cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        cls_tokens_2 = self.cls_token_2.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        x = torch.cat((cls_tokens, cls_tokens_2, x), dim=1)
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
                ...
                self.state_dict()['cls_token_2'].copy_(v)
                count += 1
            try:
                self.state_dict()[k].copy_(v)
                count += 1
    ...
    def resize_pos_embed(posemb, posemb_new, hight, width):
        ...
        posemb = torch.cat([posemb_token, posemb_token, posemb_grid], dim=1)
        return posemb
...
def vit_base_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, norm='LN', **kwargs):
    model = TransReID(
...
def vit_base_patch32_224_TransReID(img_size=(256, 128), stride_size=32, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, norm='LN', **kwargs):
    model = TransReID(
...
def vit_small_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_rate=0., attn_drop_rate=0.,drop_path_rate=0.1, norm='LN', **kwargs):
...
def deit_small_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.0, drop_rate=0.0, attn_drop_rate=0.0, norm='LN', **kwargs):
    model = TransReID(
...
def deit_tiny_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.0, drop_rate=0.0, attn_drop_rate=0.0, norm='LN', **kwargs):
    model = TransReID(
