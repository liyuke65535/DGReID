...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.patch_embed = PatchEmbed_overlap(
                img_size=img_size, patch_size=patch_size, stride_size=patch_size, in_chans=in_chans,
                embed_dim=embed_dim)
    ...
    def forward_features(self, x, mask=None):
        ...
        x_vis = x + self.pos_embed.type_as(x).to(x.device).clone().detach()
...
class mask_vit_decoder(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self, patch_size=16, num_classes=768, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
...
class color_vit_decoder(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self, patch_size=16, num_classes=768, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
...
class PretrainVisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        # self.encoder = PretrainVisionTransformerEncoder(### problems
        self.encoder = PretrainVisionTransformerEncoder(
            ...
            drop_path_rate=drop_path_rate, ##### avoid overfitting
        ...
        self.pos_embed = self.encoder.pos_embed.detach()
    ...
    def forward(self, x, mask=None, vis=False):
        ...
        mask = mask.bool()
        x_enc = self.encoder(x, mask) # [B, N_vis, C_e]
        x_vis = self.encoder_to_decoder(x_enc) # [B, N_vis, C_d]
...
def pretrain_mae_base_patch16_224(pretrained=False, img_size=224, stride_size=16, **kwargs):
    model = PretrainVisionTransformer(
        img_size=img_size, 
