...
@DATASET_REGISTRY.register()
class Market1501_gen(ImageDataset):
    """Market1501.
    ...
    def __init__(self, root='datasets', market1501_500k=False, **kwargs):
        ...
        train = self.process_dir(self.train_dir)
        train += self.process_dir_gen("/home/liyuke/data5/datasets/market1501_cp/bounding_box_train_gen") ### extra generated data
        # pids_before = set()
        # pids_after = set()
        # for obj in train:
        #     pids_before.add(obj[1])
        # for obj in train_extra:
        #     pids_after.add(obj[1])
        query = self.process_dir(self.query_dir, is_train=False)
        gallery = self.process_dir(self.gallery_dir, is_train=False)
        if self.market1501_500k:
            gallery += self.process_dir(self.extra_gallery_dir, is_train=False)

        super(Market1501_gen, self).__init__(train, query, gallery, **kwargs)
    ...
    def process_dir_gen(self, dir_path, is_train=True):
        img_paths = glob.glob(osp.join(dir_path, '*.png'))
        pattern = re.compile(r'([-\d]+)_gen')
        ...
        for img_path in img_paths:
            ...
            if is_train:
                pid = self.dataset_name + "_" + str(pid)
            data.append((img_path, pid, -1)) ## camid=-1, represent generated image
        return data
